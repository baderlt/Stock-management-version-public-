...
export default function AsideDashboardStock(props){
    ...
    const [className,setclassname]=useState({retrait:' -mb-6',stock:'-mb-6  bg-gray-800 text-red-600 rounded-md ',historique:'-mb-6',config:'-mb-6',bilan:'-mb-6',support:'-mb-6 '})
    return(

        <div className="basis-1/5 border-r border-gray-300 border-r-1 flex flex-col py-4 px-4
        bg-blue-800" >
           <b>  {isload? <div className="text-white"><FontAwesomeIcon icon={faUser}/>&ensp;{user}</div>:''}</b>
            <ul className="flex flex-col gap-y-6 pt-10 pl-0 ">
            <li className={className.stock} 
            onClick={()=>{props.ajout('stock'),setclassname({retrait:'-mb-6 ',stock:'-mb-6 bg-gray-800 rounded-md',historique:'-mb-6',config:'-mb-6',bilan:'-mb-6',support:'-mb-6'})}}>
                    <ButtonAsideDashboard value="Stock" 
                    iconInTag={<FontAwesomeIcon icon={faStoreAlt}/>} />
             
                </li>
                <hr />
                <li className={className.retrait}  onClick={()=>{props.ajout('retrait'),setclassname({retrait:'-mb-6 bg-gray-800 rounded-md ',stock:'-mb-6',historique:'-mb-6',config:'-mb-6',bilan:'-mb-6',support:'-mb-6'})}}>
                    <ButtonAsideDashboard value="Retrait"  iconInTag={<FontAwesomeIcon icon={faPen}/>}
                  />
                </li>
                <hr />
                <li className={className.bilan} onClick={()=>{props.ajout('bilan'),setclassname({retrait:'-mb-6',stock:'-mb-6',historique:'-mb-6',config:'-mb-6',bilan:'-mb-6  bg-gray-800 rounded-md',support:'-mb-6'})}}>
                    <ButtonAsideDashboard value="Bilan" iconInTag={<FontAwesomeIcon icon={faChartLine}/>}/>
                </li>
                <hr />
                <li className={className.historique} onClick={()=>{props.ajout('historique'),setclassname({retrait:'-mb-6',stock:'-mb-6',historique:'-mb-6  bg-gray-800 rounded-md',config:'-mb-6',bilan:'-mb-6',support:'-mb-6'})}}>
                    <ButtonAsideDashboard value="Historique" iconInTag={<FontAwesomeIcon icon={faHistory}/>}
            />
                </li>
                <hr />
                <li className={className.config} onClick={()=>{props.ajout('Configuration'),setclassname({retrait:'-mb-6',stock:'-mb-6',historique:'-mb-6',config:'-mb-6  bg-gray-800 rounded-md',bilan:'-mb-6',support:'-mb-6'})}}>
                    <ButtonAsideDashboard value="Configuration" iconInTag={<FontAwesomeIcon icon={faGear}/>}/>
                </li>
                <hr />
                <li className={className.support} onClick={()=>{props.ajout('support'),setclassname({retrait:'-mb-6',stock:'-mb-6',historique:'-mb-6',config:'-mb-6 ',bilan:'-mb-6',support:'-mb-3 bg-gray-800 rounded-md'})}}>
